...
% here head field already averaged over island volume, so have 'slices'
% of field values for several values of parameter z, to interpolate in 3D
            
if var_prop(3)==0 || var_prop(3)==3 || var_prop(3)==4 || var_prop(3)==5
    ...
    if length(x_data)==1
        % only 1 scan line, so we perform a 1D interpolation
